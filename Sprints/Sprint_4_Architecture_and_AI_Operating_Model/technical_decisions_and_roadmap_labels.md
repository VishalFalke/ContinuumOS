...
| TDR-16 | Treat openEHR as a user-provided research influence, not the selected MVP interoperability or persistence architecture. Use simulated SMART on FHIR and FHIR R4-shaped reads for the bounded application-access demonstration while source systems remain authoritative. | Deferred / excluded for implementation | openEHR's separation of clinical information models from application logic and its longitudinal-record orientation informed the broader research, but ContinuumOS solves workflow continuity rather than building a longitudinal clinical repository. No archetype, template, CDR or openEHR conformance is implemented or claimed. | Revisit only if a separately approved longitudinal-record/data-platform need, governed clinical models, participant architecture and conformance plan are established. |
...
- Portfolio context: `hospital_systems_discovery_and_problem_coverage.md` records the wider hospital-system problem landscape and the bounded openEHR-to-FHIR rationale without changing the MVP.
